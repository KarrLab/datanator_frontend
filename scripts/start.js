...
const { scriptVersion } = require('react-app-rewired/scripts/utils/paths');
...
checkBrowsers(paths.appPath, isInteractive)
  ...
  .then(port => {
    ...
    /* socket server to listen for stop signals */
    const express = require('express');
    const app = express();
    const stopServer = require('http').createServer(app);
    stopServer.listen(port + 1);
    const io = require('socket.io')(stopServer);
    io.on('connection', (stopServer) => {
      stopServer.on('npmStop', () => {
        process.exit(0);
      });
    });
  })
