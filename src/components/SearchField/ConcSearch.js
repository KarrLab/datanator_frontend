...
"Homo sapiens",
	...
	handleClickInner(){
		...
		this.props.handleClick("/metabconcs/" + this.state.molecule + "/" + this.state.organism)
	}
    ...
    return (
      ...
      <img src={require('~/images/search_logo.png')} />
  	  ...
  	  {!(this.props.landing) &&
      <img src={require('~/images/DT.png')} />
  		}

        <InputGroup compact >
          <Input style={{ width: '30%' }} defaultValue={this.props.defaultMolecule} addonBefore="Molecule" onChange={event => { this.setState({ molecule: event.target.value })  }}/>
           <AutoComplete
        dataSource={this.state.dataSource}
        defaultValue={this.props.defaultOrganism}
        style={{ width: '30%' }}
        onSelect={value => { this.setState({ organism: value })}}
        addonBefore="Organism"
        label="organism"
        placeholder="input here"
        onChange={value => { this.setState({ organism: value })}}
        filterOption={(inputValue, option) =>
        option.props.children.toUpperCase().indexOf(inputValue.toUpperCase()) !== -1}
        >
        	<Input  defaultValue="" addonBefore="Organism"  />
	      </AutoComplete>
          <Button type="primary" shape="circle" icon="search" onClick={this.handleClickInner}/>
        </InputGroup>
